...
// library.gs
const TrackUtils = (function () {
  return {
    getAndProcessTracks,
  };
  async function getAndProcessTracks(
    trackSources,
    savedTracks,
    bannedTracks,
    limit
    ...
    try {
      // Fetch tracks from all sources in parallel.
      const trackPromises = trackSources.map((source) =>
        Source.getPlaylistTracks(source.name, source.id)
      );
      const allTracksArrays = await Promise.all(trackPromises);

      // Combine all tracks into a single array.
      let allTracks = allTracksArrays.flat();

      // Remove saved tracks.
      Filter.removeTracks(allTracks, savedTracks, true);

      // Remove banned tracks.
      ...
      Selector.keepRandom(allTracks, limit);

      return allTracks;
    } catch (error) {
      console.error("Error in getAndProcessTracks:", error);
...
function displayLaunchPage_() {
  try {
    ...
    return HtmlService.createHtmlOutput("Authorization was successful");
  }
}
...
Array.prototype.toObject = function (parseMethod) {
  return this.reduce(
    (accumulator, element, i) => (
      (accumulator[parseMethod(element)] = i), accumulator
    ),
    {}
  );
};
...
JSON.parseFromString = function (content) {
  try {
    ...
    Admin.printError(
      "Failed to convert JSON string to JavaScript object\n",
      error.stack,
      "\nContent:",
      `"${content}"`
    );
...
JSON.parseFromResponse = function (response) {
  ...
  if (content.length == 0) {
    return { msg: "Empty response body", status: response.getResponseCode() };
  }
...
const CustomUrlFetchApp = (function () {
  ...
  function readResponse(response, url, params = {}) {
    ...
    function writeErrorLog() {
      ...
      if (errorText.toLowerCase().includes("access not granted")) {
        throw `No access rights. To reauthorize:\n1) Click "start deployment > trial deployments"\n2) Follow the link "web application"\n3) Confirm access rights\n\nThe error may occur when changing the password for the Spotify account.\n\nIf the web application page does not open when following the link, repeat the steps in incognito mode\n(Google automatically redirects to another account where your project with goofy is not available).`;
      } else {
        Admin.printError(
  ...
  function parseQuery(obj) {
    return Object.keys(obj)
      .map((k) =>
        (typeof obj[k] != "string" && obj[k] != undefined) ||
...
const Audiolist = (function () {
  ...
  const VARIABLE_TYPES = {
    SPOTIFY_TRACK: {
      entityType: "TRACK",
      platform: "SPOTIFY",
      classType: "SpotifyGoofyTrack",
    },
    LASTFM_TRACK: {
      entityType: "TRACK",
      platform: "LASTFM",
      classType: "LastfmGoofyTrack",
    },
  };
  ...
  return {
    ...
    onPost(args) {
      try {
        ...
        if (func == undefined) {
          return Audiolist.responseMessage(
            `Failed to find function with name ${data.funcName}. Check the case of the letters and update the deployment.`,
            Audiolist.MESSAGE_TYPES.ERROR
          );
        }
        return func(data);
      } catch (e) {
    ...
    hello() {
      return responseMessage(
        'Hello world! You managed to connect goofy and Audiolist. You can run any functions and send responses. For example, the built-in function "Audiolist.history" will send all tracks from the goofy listening history. To use them in the next commands, add an output variable for the "Goofy function" command.'
      );
    },
    ...
    history() {
      ...
      return responseItems(
        recentTracks,
        Audiolist.VARIABLE_TYPES.SPOTIFY_TRACK
      );
    },
  };
})();
...
const Source = (function () {
  ...
  async function applySeasonalTracks(tracks, isSpookySeason, isFestiveSeason) {
    console.log("Starting applySeasonalTracks");
    try {
      if (!tracks) {
        console.error("applySeasonalTracks: tracks is undefined");
        throw new Error("applySeasonalTracks: tracks is undefined");
      }

      const seasonalTrackPromises = [];

      if (isSpookySeason) {
        seasonalTrackPromises.push(
          (async () => {
            const spookyTracks = await getPlaylistTracks(
              "spooky",
              "6QyTU1QD9mP7WIx0SSmnXL"
            );
            if (!spookyTracks) {
              throw new Error("applySeasonalTracks: spookyTracks is undefined");
            }
            return spookyTracks;
          })()
        );
      ...
      if (isFestiveSeason) {
        seasonalTrackPromises.push(
          (async () => {
            const festiveTracks = await getPlaylistTracks(
              "festive",
              "2Iho7W6TYn4eufLQZ5GZyx"
            );
            if (!festiveTracks) {
              throw new Error(
                "applySeasonalTracks: festiveTracks is undefined"
              );
            }
            return festiveTracks;
          })()
        );
      }

      // Wait for all seasonal track promises to resolve.
      const seasonalTracksArrays = await Promise.all(seasonalTrackPromises);

      // Combine all seasonal tracks with the main tracks array.
      seasonalTracksArrays.forEach((seasonalTracks) => {
        Combiner.push(tracks, seasonalTracks);
      });

      console.log("Finished applySeasonalTracks");
      return tracks;
    } catch (error) {
      console.error("Error in applySeasonalTracks:", error);
      throw error; // Re-throw the error to be handled by the caller.
    }
  }
  ...
  function getTop(timeRange, type) {
    // Spotify bug: https://community.spotify.com/t5/Spotify-for-Developers/Bug-with-offset-for-method-quot-Get-User-s-Top-Artists-and/td-p/5032362
  ...
  function getArtistsTopTracks(artists, isFlat = true) {
    return getArtistsByPath(
      artists,
      "/artists/%s/top-tracks?market=from_token",
      isFlat
    );
  }
  ...
  function getRecomArtists(artists, queryObj = {}, isFlat = true) {
    ...
    let result = recomTrackGroups.map((response) =>
      response.tracks.reduce((acc, track) => {
  ...
  function getFollowedTracks(params = {}) {
    let playlists = getFollowedPlaylists(
      params.type,
      params.userId,
      params.exclude
    );
    Selector.keepRandom(playlists, params.limit);
    return !params.hasOwnProperty("isFlat") || params.isFlat
      ? getTracks(playlists)
      : playlists.map((p) => {
  ...
  function getArtistsTracks(params) {
    ...
    let tracks = getArtistsAlbums(artists, params.album).reduce(
      (tracks, albums) => {
  ...
  function getArtistsAlbums(artists, params = {}) {
    ...
    let albums = artists.reduce((albums, artist) => {
      ...
      let response = SpotifyRequest.getItemsByPath(path).filter((album) =>
        RangeTracks.isBelongReleaseDate(album.release_date, params.release_date)
      );
  ...
  function getSavedAlbumTracks(limit) {
    ...
    return items.reduce((tracks, album) => {
      let albumTracks = album.tracks.items.map(
        (item) => ((item.album = album), item)
      );
      // Fix for error when using JSON.stringify
  ...
  function getSavedAlbums() {
    return SpotifyRequest.getItemsByPath("me/albums?limit=50", 400).map(
      (item) => {
  ...
  function getRecentReleasesByArtists(params) {
    if (params.artists.length >= 300) {
      console.warn(
        `ПWith a large number of artists (requested ${params.artists.length}), the function may not fit within the execution time limit or receive a temporary ban from Spotify for spamming requests.\n\nUse the template: https://chimildic.github.io/goofy/#/template?id=New-releases-in-parts`
      );
    }
    let initDateStr = new Date().toISOString();
    let startDate = params.hasOwnProperty("sinceDays")
      ? Filter.getDateRel(params.beforeDays, "startDay")
      : params.startDate;
    let types = params.type.sort((a, b) => a.localeCompare(b));
    let requestUrls = params.artists.map(
      (artist) =>
        `${API_BASE_URL}/artists/${
          artist.id
        }/albums?include_groups=${types.join(",")}&limit=50&market=from_token`
    );

    let foundReleases = {};
    let uncompleteRequests = [];
    SpotifyRequest.getAll(requestUrls).forEach((response, index) => {
      let minFoundDateStrings = types.reduce(
        (acc, type) => ((acc[type] = initDateStr), acc),
        {}
      );
    ...
    uncompleteRequests.forEach((item) => {
      let artistAlbums = Source.getArtistsAlbums(
        /* artist = { id: '' }*/ item,
        /* params = { groups: [] }*/ item
      );
  ...
  function mineTracks(params) {
    ...
    let selectMethod =
      params.hasOwnProperty("inRow") && params.inRow
        ? Selector.keepFirst
        : Selector.keepRandom;
  ...
  function craftTracks(tracks, params = {}) {
    let recomTracks = SpotifyRequest.getAll(createUrls()).reduce(
      (recomTracks, response) => {
    ...
    function findAvailablePosition() {
      ...
      if (
        params.query.hasOwnProperty("seed_tracks") &&
        params.key != "seed_tracks"
      ) {
        value -= params.query.seed_tracks.split(",").length;
      }
      if (
        params.query.hasOwnProperty("seed_artists") &&
        params.key != "seed_artists"
      ) {
        value -= params.query.seed_artists.split(",").length;
      }
      if (value <= 0) {
        throw `Не осталось места под ${params.key}. Reduce the number of values for other seed_*`;
      }
      return value;
    }
  }
  ...
  async function getPlaylistTracks(name, id, userId, count, inRow) {
    console.log("Starting getPlaylistTracks");
    try {
      const tracks = await getTracks([
        {
          ...
          userId: userId,
          count: count,
          inRow: inRow == undefined ? true : inRow,
        },
      ]);
      console.log("Finished getPlaylistTracks");
      return tracks;
    } catch (error) {
  ...
  function getItemsByPlaylistObject(obj) {
    ...
    if (obj && obj.tracks && obj.tracks.items) {
      items =
        obj.tracks.total <= 100
          ? obj.tracks.items
          : SpotifyRequest.getItemsByNext(obj.tracks);
      items.forEach(
        (item) => (item.origin = { id: obj.id, name: obj.name, type: obj.type })
      );
    }
    return items;
  }
  ...
  function getSavedTracks(limit = 200000) {
    ...
    let items = SpotifyRequest.getItemsByPath(
      "me/tracks?limit=50",
      requestCount
    );
  ...
  function extractTracks(items) {
    ...
    return items.reduce((tracks, item) => {
      if (
        (!item.hasOwnProperty("is_local") || !item.is_local) &&
        item.track &&
        item.track.artists && //Added check
...
const Player = (function () {
  ...
  function transferPlayback(deviceId, isPlay) {
    return SpotifyRequest.put(`${API_BASE_URL}/me/player`, {
      device_ids: [deviceId],
      play: isPlay,
    });
  }
  ...
  function toggleShuffle(state, deviceId) {
    return SpotifyRequest.put(
      createUrl("/me/player/shuffle", deviceId, { state: state })
    );
  }

  function setRepeatMode(state, deviceId) {
    return SpotifyRequest.put(
      createUrl("/me/player/repeat", deviceId, { state: state })
    );
  }
...
const RecentTracks = (function () {
  ...
  function update() {
    ...
    if (ON_SPOTIFY_RECENT_TRACKS) {
      hasUpdate += updatePlatformRecentTracks(
    ...
    if (ON_LASTFM_RECENT_TRACKS) {
      let lastfmTracks = Lastfm.getLastfmRecentTracks(
        KeyValue.LASTFM_LOGIN,
        parseInt(KeyValue.LASTFM_RANGE_RECENT_TRACKS)
      );
      hasUpdate += updatePlatformRecentTracks(
        lastfmTracks,
        LASTFM_FILENAME,
        Lastfm.findNewPlayed
      );
    }
  ...
  function updatePlatformRecentTracks(
    recentTracks,
    filename,
    findNewPlayedMethod
  ) {
  ...
  function appendNewPlayed(newItems, filename) {
    if (newItems.length == 0) {
      Admin.printInfo(` new tracks ${filename}`);
      return false;
    }
    Cache.compressTracks(newItems);
    let total = Cache.append(filename, newItems, "begin", ITEMS_LIMIT);
    Admin.printInfo(`+${newItems.length}, total: ${total} (${filename})`);
    return true;
  }
...
const Combiner = (function () {
  ...
  function mixinMulti(params) {
    let resultArray = [];
    if (
      params.source.length != params.inRow.length ||
      (params.toLimitOn && !isEnoughItems())
    ) {
      return resultArray;
    }
...
const RangeTracks = (function () {
  ...
  function rangeTracks(tracks, params) {
    ...
    let filteredTracks = tracks.filter((track) => {
      if (
        isBelongMeta(track) &&
        isBelongFeatures(track) &&
        isBelongArtist(track) &&
        isBelongAlbum(track)
      ) {
        return true;
      } else {
  ...
  function isBelongMeta(track) {
    ...
    let trackMeta = _cachedTracks.meta[track.id]
      ? _cachedTracks.meta[track.id]
      : track;
  ...
  function isBelongArtist(track) {
    ...
    return (
      ...
      isBelongGenres(
        trackArtist.genres,
        _params.artist.genres,
        _params.artist.isRemoveUnknownGenre
      ) &&
      !isBelongBanGenres(
        trackArtist.genres,
        _params.artist.ban_genres,
        _params.artist.isRemoveUnknownGenre
      )
    );
  }
  ...
  function isBelongReleaseDate(albumReleaseDate, targetPeriod) {
    ...
    if (
      releaseDateTime < startDate.getTime() ||
      releaseDateTime > endDate.getTime()
    ) {
      return false;
    }
    return true;
    ...
    for (let key in args) {
      if (BAN_KEYS.includes(key)) {
        ...
        (Array.isArray(args[key]) &&
          !args[key].some((value) => value == obj[key])) ||
        (typeof args[key] == "object" &&
          (obj[key] < args[key].min || obj[key] > args[key].max)) ||
...
const Filter = (function () {
  function removeUnavailable(tracks, market) {
    ...
    function identifyState() {
      tracks.forEach((t) => {
        if (
          t.hasOwnProperty("available_markets") &&
          t.available_markets.includes(market)
        ) {
    ...
    function defineUnavailableState() {
      ...
      SpotifyRequest.getFullObjByIds(
        "tracks",
        unclearState,
        50,
        market
        ...
        if (!t) {
          ...
          Admin.printInfo(
            `The track's ID has changed, old value ${id} (${getTrackKeys(
              track
            )})`
          );
    ...
    function removeUnavailableTracks() {
      ...
      let availableTracks = tracks.filter(
        (t) => !unavailableState.includes(t.id)
      );
  ...
  function removeTracks(original, removable, invert = false, mode = "every") {
    ...
    let filteredTracks = original.filter(
      (item) =>
        invert ^
  ...
  function removeArtists(original, removable, invert = false, mode = "every") {
    let artists = removable
      .map((item) =>
        item.artists ? (mode == "every" ? item.artists : item.artists[0]) : item
      )
      .flat(1);
    let ids = artists.toObject((item) => item.id);
    let filteredTracks = original.filter(
      (item) =>
        invert ^ !getArtistIds(item, mode).some((id) => ids.hasOwnProperty(id))
    );
    Combiner.replace(original, filteredTracks);
  }
  ...
  function getTrackKeys(track, mode) {
    if (!track.artists || track.artists.length === 0) {
      console.error("getTrackKeys: track.artists is undefined or empty", track);
      return [];
    }
    return mode == "every"
      ? track.artists.map((artist) =>
          `${artist.name} ${track.name}`.formatName()
        )
  ...
  function replaceWithSimilar(params) {
    ...
    copyTracks.forEach((t) => {
      ...
      let params = {
        seed_tracks: t.id,
        seed_artists: Selector.sliceFirst(
    ...
    let resultTracks = params.origin
      .map((track) => {
        if (keys.includes(track.id)) {
          if (similarTracks[track.id].length > 0) {
            ...
            track = Selector.sliceRandom(
              similarTracks[track.id],
              params.count || 1
            );
  ...
  function matchOriginalOnly(items) {
    matchExcept(
      items,
      "mix|club|radio|piano|acoustic|edit|live|version|cover|karaoke"
    );
  }
      ...
      if (typeof item == "undefined") {
        ...
        return (
          ...
          (regex.test(item.name.formatName()) ||
            ...
            (item.album.artists &&
              item.album.artists.every((a) => regex.test(a.name.formatName()))))
        );
      }
  ...
  function detectLanguage(tracks, params) {
      ...
      Combiner.replace(
        ...
        tracks.filter((track) => {
          if (![undefined, "und", "#ERROR!"].includes(track.lyrics.lang)) {
            return (
              RangeTracks.isBelongGenres([track.lyrics.lang], params.include) &&
              !RangeTracks.isBelongBanGenres(
                [track.lyrics.lang],
                params.exclude
              )
            );
    ...
    function getLyrics() {
      ...
      return CustomUrlFetchApp.fetchAll(urls).some((response, i) => {
        ...
        if (json.message.header.status_code == 404) {
          Admin.printInfo(
            "Не найден текст для",
            `${unknownLyrics[i].artists[0].name} - ${unknownLyrics[i].name}`
          );
          unknownLyrics[i].lyrics.lang = "und";
        } else if (json.message.header.status_code != 200) {
          Admin.printError(
            "Ошибка при получении текста",
            "The limit from musixmatch may have been exceeded..",
            "Code:",
            json.message.header.status_code
          );
          return true;
        }
      });
    ...
    function detectLanguage() {
      ...
      if (!row || row.length == 0) {
        Admin.printInfo(
          "Zero tracks for language recognition among",
          tracks.length,
          "треков"
        );
        return;
      ...
      sheet
        ...
        .forEach((c, i) => {
          if (c == "#ERROR!") {
            Admin.printError(
              "Error while trying to identify the language::",
              unknownLang[i].lyrics.text
            );
          }
  ...
  function extractItemsAbs(items, startDate, endDate) {
    ...
    if (startTime >= endTime) {
      Admin.printError(
        "Left boundary is greater than right:",
        startDate,
        endDate
      );
      return;
  ...
  const Deduplicator = (function () {
    ...
    function findTracksDuplicated(offsetDurationMs) {
      ...
      _duplicates = _items.reduce((duplicates, track, index) => {
        if (!track || track === null || track.id === null) {
          // Added check
          return duplicates;
        } else if (
          seenIds.hasOwnProperty(track.id) ||
          isDuplicateByName(track)
        ) {
          duplicates.push({
            index: index,
            track: track,
            reason: seenIds.hasOwnProperty(track.id)
              ? "same-track-id"
              : "same-track-name",
          });
        } else {
      ...
      function isDuplicateByName(track) {
        return getTrackKeys(track).some(
          (key) =>
            seenTrackKeys.hasOwnProperty(key) &&
            seenTrackKeys[key].filter(
              (duration) =>
                Math.abs(duration - track.duration_ms) < offsetDurationMs
            ).length > 0
        );
      }
    }
    ...
    function findAlbumsDuplicated() {
      ...
      function getAlbumId(item) {
        try {
          return item.album
            ? `${item.artists[0].name} ${item.album.name}`.formatName() +
                ` ${item.album.release_date} ${item.album.total_tracks}`
...
const Selector = (function () {
  ...
  function pickYear(tracks, year, offset = 5) {
    ...
    Admin.printInfo(
      `Среди ${tracks.length} треков нет вышедших в ${year} году`
    );
    year = parseInt(year);
    let keys = Object.keys(tracksByYear).map((item) => parseInt(item));
    let nearYear = keys.sort(
      (x, y) => Math.abs(year - x) - Math.abs(year - y)
    )[0];
    if (typeof nearYear != "undefined" && Math.abs(nearYear - year) <= offset) {
      Admin.printInfo(`Closest year selected: ${nearYear}`);
      return tracksByYear[nearYear.toString()];
    }
    Admin.printInfo(
      `With an offset of ${offset}, the nearest year was not found`
    );
    return [];
  }
})();
...
const Order = (function () {
  function shuffle(array, factor = 1.0) {
    let indexArray = Array.from(Array(array.length).keys()).sort(
      (_) => Math.random() - 0.5
    );
  ...
  const sort = (function () {
    ...
    function sortArtist() {
      ...
      function getItem(x) {
        ...
        return (
          items[artist.id] ||
          arrayItems.find((i) => i.name == artist.name) ||
          {}
        );
      }
    }
    ...
    function sortAlbum() {
      ...
      if (_key == "name") {
        _source.sort((x, y) =>
          compareString(items[extract(x).id], items[extract(y).id])
        );
      } else if (_key == "release_date") {
        _source.sort((x, y) =>
          compareDate(items[extract(x).id], items[extract(y).id])
        );
      } else if (_key == "popularity") {
        _source.sort((x, y) =>
          compareNumber(items[extract(x).id], items[extract(y).id])
        );
      }
  ...
  function separateArtists(items, space, isRandom = false) {
    ...
    function tryInsert(item) {
      for (let i = 0; i <= original.length; i++) {
        ...
        let endIndex =
          i + space < original.length ? i + space : original.length;
  ...
  function separateYears(tracks) {
    return tracks.reduce((tracksByYear, track) => {
      if (
        ...
        track.album.hasOwnProperty("release_date")
      ) {
...
const Playlist = (function () {
  ...
  const getPlaylistArray = (function () {
    ...
    function get(userId) {
      ...
      if (playlistsOfUsers[key] == null) {
        let path =
          userId == null ? "me/playlists" : `users/${userId}/playlists`;
  ...
  function modifyTracks(requestType, data) {
      ...
      if (SpotifyRequest[requestType](url, payload) != undefined) {
        ...
        if (attempt == 1) {
          ...
          if (Playlist.getById(data.id) == undefined) {
            throw `Unable to add tracks. Playlist with id ${data.id} does not exist. Correct the id in the Playlist.save* function`;
          }
        }
        i--; // повторить запрос
        Admin.pause(5);
      } else {
        Admin.printInfo("Track addition aborted after 3 failed attempts");
        break;
      }
    }
    ...
    function createTrackBackup() {
      ...
      Admin.printInfo(
        `A backup copy of the tracks has been created: ${filepath}`
      );
    }
  }
  ...
  function changeCover(data) {
    ...
    if (data.hasOwnProperty("sourceCover")) {
      ...
      data.randomCover == "update" ||
      (data.randomCover == "once" && hasMosaicCover(data.id))
    ) {
      img = getRandomCover();
    }
  ...
  function hasMosaicCover(playlistId) {
    ...
    return (
      playlist.images.length > 0 && playlist.images[0].url.includes("mosaic")
    );
  }
  ...
  function createPayload(data) {
    ...
    if (data.description) {
      payload.description = Selector.sliceFirst(
        data.description,
        LIMIT_DESCRIPTION
      );
    }
...
const Library = (function () {
  ...
  function modifyFollowPlaylists(method, playlists) {
    ...
    let urls = playlists.map(
      (p) => `${API_BASE_URL}/playlists/${p.id}/followers`
    );
...
const Lastfm = (function () {
  ...
  function getSimilar(params) {
    ...
    function createRequests() {
      return params.items.reduce((requests, item) => {
        ...
        requests.push({
          url: LASTFM_API_BASE_URL + CustomUrlFetchApp.parseQuery(queryStr),
        });
        return requests;
      }, []);
    }
    ...
    function getAll() {
      return CustomUrlFetchApp.fetchAll(requests).reduce(
        (similarItems, response) => {
          if (response[params.parentKey]) {
            let filteredItems = response[params.parentKey][
              params.childKey
            ].filter((item) => item.match >= params.match);
            _ = params.isFlat
              ? Combiner.push(similarItems, filteredItems)
              : similarItems.push(filteredItems);
          }
          return similarItems;
        },
        []
  ...
  function removeRecentTracks(original, user, count = 600, mode = "every") {
    ...
    let removableNames = removableTracks.map((item) =>
      formatLastfmTrackKey(item)
    );
    let filteredTracks = original.filter(
      (item) =>
        !formatSpotifyTrackKeys(item, mode).some((name) =>
          removableNames.includes(name)
        )
    );
    Combiner.replace(original, filteredTracks);
  }
  ...
  function removeRecentArtists(original, user, count = 600, mode = "every") {
    let removableTracks = getLastfmRecentTracks(validUser(user), count);
    let removableNames = removableTracks.map((item) =>
      item.artist["#text"].formatName()
    );
    let filteredTracks = original.filter(
      (item) =>
        !formatSpotifyArtistKeys(item, mode).some((name) =>
          removableNames.includes(name)
        )
    );
    Combiner.replace(original, filteredTracks);
  ...
  function findNewPlayed(lastfmTracks, spotifyTracks) {
    ...
    function sliceNewPlayed() {
      ...
      return lastfmTracks.filter((lastfmItem) => {
        return (
          -1 ==
          spotifyTracks.findIndex(
            (spotifyItem) => spotifyItem.played_at == lastfmItem.date["#text"]
          )
        );
      });
    }
  ...
  function getLovedTracks(user, limit) {
    ...
    if (!tracks.lovedtracks) {
      Admin.printError("Error getting loved tracks", tracks);
      return [];
    }
    return Search.multisearchTracks(
      tracks.lovedtracks.track,
      formatTrackNameLastfm
    );
  }
  ...
  function getTopTracks(params) {
    ...
    if (!tracks.toptracks) {
      Admin.printError("Error getting top tracks", tracks);
      return [];
    }
    return Search.multisearchTracks(
  ...
  function getTopArtists(params) {
    ...
    if (!artists.topartists) {
      Admin.printError("Error getting top artists", artists);
      return [];
    }
    return Search.multisearchArtists(
      artists.topartists.artist,
      formatArtistNameLastfm
    );
  }
  ...
  function getTopAlbums(params) {
    ...
    if (!albums.topalbums) {
      Admin.printError("Error getting top albums", albums);
      return [];
    }
    return Search.multisearchAlbums(
      albums.topalbums.album,
      formatAlbumNameLastfm
    );
  }
  ...
  function getCustomTop(params) {
    ...
    played = played.filter(
      (p) => p.count >= params.minPlayed && p.count <= params.maxPlayed
    );
    ...
    return getItemsFromTagPage({
      ...
      callback: (cheerio) => {
        ...
        cheerio(
          ...
          let album = cheerio(node)
            .children(".resource-list--release-list-item-name")
            .text()
            .trim();
          let artist = cheerio(node)
            .children(".resource-list--release-list-item-artist")
            .text()
            .trim();
  ...
  function getArtistsByTag(tag, limit) {
    return getItemsFromTagPage({
      ...
      callback: (cheerio) => {
        let names = [];
        cheerio(".big-artist-list-title", "", cheerio(".big-artist-list")).each(
          (index, node) => {
  ...
  function getTracksByTag(tag, limit) {
    return getItemsFromTagPage({
      ...
      callback: (cheerio) => {
        let names = [];
        cheerio(".chartlist-row", "", cheerio(".chartlist")).each(
          (index, node) => {
            let name = cheerio(node).children(".chartlist-name").text().trim();
            let artist = cheerio(node)
              .children(".chartlist-artist")
              .text()
              .trim();
  ...
  function getItemsFromTagPage(params) {
    ...
    function parseNames() {
      ...
      for (let i = 0; i < pageCount; i++) {
        urls.push(
          `https://www.last.fm/tag/${params.tag}/${params.type}s?page=${i + 1}`
        );
      }
      let names = [];
  ...
  function rangeTags(spotifyTracks, params) {
    spotifyTracks.forEach((t) => {
      ...
      if (response.toptags && response.toptags.tag) {
        if (response.toptags.tag.length > 0) {
          ...
          Admin.printInfo("The track has no tagsв", trackname);
        }
    ...
    Combiner.replace(
      spotifyTracks,
      spotifyTracks.filter((t) => {
        if (!t.hasOwnProperty("tags") || t.tags.length == 0) {
          return !params.isRemoveUnknown;
        }
        return (
          isSomeBelong(t, params.include, true) &&
          !isSomeBelong(t, params.exclude, false)
        );
      })
    );
...
const Search = (function () {
  ...
  let minDiceRating = KeyValue.hasOwnProperty("MIN_DICE_RATING")
    ? parseFloat(KeyValue.MIN_DICE_RATING)
    : 0.6005;
  let noFound = [];
  ...
  const Translit = (function () {
    ...
    function transform(inputStr) {
      return inputStr
        .split("")
        .map((s) => (TABLE.hasOwnProperty(s) ? TABLE[s] : s))
        .join("");
    }
  })();
  ...
  return {
    DicesCoefficient: DiceCoefficient,
    multisearchTracks,
    multisearchArtists,
    multisearchAlbums,
    findPlaylists,
  ...
  function multisearch(items, type, parseNameMethod) {
    ...
    let originKeywords = parseNameMethod
      ? items.map((item) => parseNameMethod(item))
      : items;
      ...
      for (let i = 0; i < uniqueKeywords.length; i++) {
        ...
        if (item && item.id) {
          ...
          noFound.push({
            ...
            item: items[
              originKeywords.findIndex((item) => item == uniqueKeywords[i])
            ],
          });
          Admin.printInfo(
            "Spotify by type",
  ...
  function findBest(keywords, type) {
    let urls = keywords.map((keyword) => {
      ...
      return Utilities.formatString(
        TEMPLATE,
        CustomUrlFetchApp.parseQuery(queryObj)
      );
    });
    return SpotifyRequest.getAll(urls).map((response, index) => {
      ...
      let targetStrings = response.items
        .filter((item) => {
          return item != undefined;
        })
        .map((item) => {
          let str = (
            type == "track" ? `${item.artists[0].name} ${item.name}` : item.name
          ).formatName();
      ...
      let result = DiceCoefficient.findBestMatch(
        Translit.transform(keywords[index]),
        targetStrings
      );
      return result.bestMatch.rating >= minDiceRating
        ? response.items[result.bestMatchIndex]
        : {};
    });
  }
...
const getCachedTracks = (function () {
  ...
  function findIdsOfUncachedObj() {
    _tracks.forEach((sourceItem) => {
      ...
      if (
        _args.meta &&
        !cachedTracks.meta[sourceItem.id] &&
        isSimplified(sourceItem)
      ) {
      ...
      if (
        _args.artist &&
        !cachedTracks.artists[targetItem.id] &&
        isSimplified(targetItem)
      ) {
      ...
      if (
        _args.album &&
        !cachedTracks.albums[targetItem.id] &&
        isSimplified(targetItem)
      ) {
  ...
  function cacheToFullObj() {
    if (uncachedTracks.meta.length > 0) {
      let fullTracks = SpotifyRequest.getFullObjByIds(
        "tracks",
        uncachedTracks.meta,
        50
      );
      fullTracks.forEach((track, i) =>
        isNull(track, uncachedTracks.meta[i], "meta")
          ? false
          : (cachedTracks.meta[track.id] = track)
      );
    }
    if (uncachedTracks.artists.length > 0) {
      let fullArtists = SpotifyRequest.getFullObjByIds(
        "artists",
        uncachedTracks.artists,
        50
      );
      fullArtists.forEach((artist, i) =>
        isNull(artist, uncachedTracks.artists[i], "artist")
          ? false
          : (cachedTracks.artists[artist.id] = artist)
      );
    }
    if (uncachedTracks.albums.length > 0) {
      let fullAlbums = SpotifyRequest.getFullObjByIds(
        "albums",
        uncachedTracks.albums,
        20
      );
      fullAlbums.forEach((album, i) =>
        isNull(album, uncachedTracks.albums[i], "album")
          ? false
          : (cachedTracks.albums[album.id] = album)
      );
    }
    if (uncachedTracks.features.length > 0) {
      ...
      let features = SpotifyRequest.getFullObjByIds(
        "audio-features",
        uncachedTracks.features,
        85
      );
      features.forEach((item, i) => {
        if (item == null) {
          ...
          Admin.printInfo(`Track has no features, id: ${id}`);
        } else {
  ...
  function isNull(item, sourceId, type) {
    !item && Admin.printInfo(`No data of type ${type} for ${sourceId}`);
    return !item;
  }
})();
...
const Auth = (function () {
  ...
  function createService(name, isPrivate) {
    ...
    if (isPrivate && KeyValue.PRIVATE_CLIENT_SECRET != undefined) {
      if (KeyValue.PRIVATE_CLIENT_SECRET.length > 32) {
        ...
        KeyValue.PRIVATE_CLIENT_SECRET =
          Utilities.newBlob(decoded).getDataAsString();
      }
      service
        .setClientId(KeyValue.PRIVATE_CLIENT_ID)
  ...
  function displayAuthResult(request) {
    ...
    return HtmlService.createHtmlOutput(
      isAuthorized ? "Успешно" : "Отказано в доступе"
    );
  }
  ...
  function displayAuthPage() {
    ...
    let html = Utilities.formatString(
      htmlTemplate,
      redirectTemplate + publicRestParams,
      redirectTemplate + privateRestParams
    );
...
const SpotifyRequest = (function () {
  ...
  function getAll(urls, allowLocale = true) {
    ...
    return CustomUrlFetchApp.fetchAll(requests).map((response) =>
  ...
  function getHeaders(url, method = "get") {
    ...
    return {
      Authorization: `Bearer ${
        isPrivate ? Auth.getPrivateAccessToken() : Auth.getPublicAccessToken()
      }`,
    };
  }
...
const Cache = (function () {
  ...
  const USER_FOLDER = User.id
    ? getFolder(ROOT_FOLDER, User.id, true)
    : ROOT_FOLDER;
  ...
  function read(filepath) {
    ...
    function getContentFromFile(attempt = 0) {
      if (attempt == 3)
        throw new Error(`Unknown error while reading file ${filepath}`);
      ...
      if (str.length == 0) {
        Admin.printInfo("Empty string from blob object");
        Admin.pause(2);
  ...
  function append(filepath, content, place = "end", limit = 200000) {
    ...
    function appendString() {
      let raw =
        place == "begin" ? content + currentContent : currentContent + content;
      write(filepath, raw);
      return raw.length;
    }
  }
  ...
  function compressTracks(tracks) {
    if (
      !(tracks && tracks.length > 0 && (tracks[0].album || tracks[0].track))
    ) {
      return;
    }
  ...
  function parsePath(filepath, isCreateFolder) {
    ...
    return [
      path.reduce(
        (root, name) => getFolder(root, name, isCreateFolder),
        rootFolder
      ),
...
const Clerk = (function () {
  ...
  function runOnceAWeek(dayStr, timeStr, callback) {
    if (
      Selector.isDayOfWeek(dayStr) &&
      isTimeToRun(callback.name, timeStr, 7)
    ) {
      return run(callback);
    }
  }
...
const Admin = (function () {
  ...
  if (VERSION != KeyValue.VERSION) {
    ...
    sendVersion(VERSION);
    ["updateSavedTracks", "updateSavedTracks_"].forEach((name) =>
      Trigger.remove(Trigger.get(name))
    );
  }
  return {
    reset,
  ...
  function pause(seconds) {
    isInfoLvl &&
      console.info(`The operation will continue after a pause of ${seconds}s.`);
  ...
  function sendVersion(value) {
    ...
    CustomUrlFetchApp.fetch(
      `https://docs.google.com/forms/u/0/d/e/${id}/formResponse`,
      {
        method: "post",
        payload: {
