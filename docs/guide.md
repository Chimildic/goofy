...
- `MUSIXMATCH_API_KEY` (строка) - ключ от сервиса musixmatch для работы функции [detectLanguage](/func?id=detectlanguage). Создается [дополнительно](/install?id=Настройка-musicmatch).
