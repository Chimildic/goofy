...
С версии 1.6.1 поддерживается создание файлов в папках. Все данные по-прежнему размещаются в корневой папке `Goofy Data`. Скрипты предыдущих версий не требуют никаких изменений.

Создаваемые папки делятся на два вида:
- _Предопределенные_. Создаются автоматически в корневой папке с именем _id_ аккаунта Spotify. На один аккаунт - одна предопределенная папка.
- _Пользовательские_. Создаются вами при использовании модуля [Cache](/func?id=cache).

?> Описываемые примеры работы с папками справедливы для всех функций `Cache`

Пример 1 - Для создания файла укажите только имя. Он будет находится в _предопределенной_ папке для текущего аккаунта.
```js
Cache.write('MySavedTracks.json', []);
```

Пример 2 - Чтобы создать _пользовательскую_ папку, укажите в строке: имя папки, слэш `/`, имя файла. Ниже пример создания файла `example.json` в папке `test`, которая разместится в _предопределенной_ папке для текущего аккаунта.
```js
...
// Папка myfolder в предопределенной папке текущего аккаунта
...
Файлы от [Cache](/func?id=cache) хранятся на [Google Диске](https://drive.google.com/). В том числе [история прослушиваний](/overview?id=История-прослушиваний). Удаленный файл попадает в корзину, где доступен еще 30 дней. Каждый файл имеет до 100 версий. Каждая запись в один и тот же файл создает новую версию. 
