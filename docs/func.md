...
Возвращает массив треков или исполнителей, отсортированных по количеству прослушиваний за указанный период.

Аргументы
- (строка) `user` - логин пользователя Lastfm, чей топ собирать
- (строка) `from` - дата старта.
- (строка) `to` - дата окончания.
- (строка) `type` - вариация результа: `track` или `artist`. По умолчанию `track`.
