...
Возвращает массив треков, найденных при поиске плейлистов по ключевым словам. Из результата удаляются дубликаты треков.

Аргументы
- (объект) `params` - параметры поиска.

Описание `params`
- (массив) `keyword` - перечень ключевых слов для поиска плейлистов.
- (число) `requestCount` - количество запросов на одно ключевое слово. С одного запроса 50 плейлистов, если они есть. Максимум 40 запросов. По умолчанию один.
- (число) `playlistCount` - количество выбираемых плейлистов из всех найденных на одно ключевое слово. По умолчанию три.
- (бул) `inRow` - если не указано или `false`, плейлисты выбираются случайно. Если `true` берутся первые `N` плейлистов (по значению `playlistCount`).
- (число) `popularity` - минимальное значение популярности трека. По умолчанию ноль.
- (объект) `followers` - диапазон количества подписчиков плейлиста (границы включительно). Фильтр до выбора `playlistCount`. Используйте только с малым количеством `requestCount`. Поскольку на каждый найденный плейлист приходится по одному дополнительному запросу для получения числа подписчиков. Перебор большего количества плейлистов заберет много запросов. Подробнее в [ограничениях](/desc?id=Ограничения).
...
Возвращает массив треков, полученных от [getRecomTracks](/func?id=getrecomtracks) для каждой пятерки элементов исходных треков. Дубликаты исходных треков игнорируются, в рекомендованных удаляются.

> Можно частисно повлиять на формируемые пятерки элементов. До вызыва функции применив одну из сортировок Order.
...
Возвращает массив треков истории прослушиваний в зависимости от [параметров](/guide?id=Параметры). Сортировка по дате прослушивания от свежих к более ранним. 
...
Оставить только треки, **добавленные** (`added_at`) за указанный период относительно сегодня. 

> Для фильтра по дате релиза используйте [rangeTracks](/func?id=rangetracks).

> ❗️ Если трек не содержит даты добавления или прослушивания, устанавливается дата 01.01.2000. Такое возможно если трек добавлен очень давно, источником является [getTopTracks](/func?id=gettoptracks) или это плейлисты "Мой микс дня #N". Для них Spotify не дает дату.
...
Оставить только треки, **добавленные** (`added_at`) за указанный абсолютный период.
