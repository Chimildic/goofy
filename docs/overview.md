...
goofy это набор функций на JavaScript. Они позволяют вам не задумываться о низкоуровневых операциях. Например, как создать плейлист и добавить треки.

Вы составляете алгоритм действий, которые исполнит платформа Google Apps Script. Грубо говоря, она позволяет создавать макросы для сервисов Google (документы, таблицы). На ряду с ними предоставляя доступ к внешним сервисам. Обладает механизмом триггеров - расписание, которое определяет когда и что выполнить.
